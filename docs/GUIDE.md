...
> **ALPHA / BREAKING RELEASE — 4.1.0-alpha**
>
> Upgrade deliberately: PiP 4.1 alpha replaces the 3.x API and tightens validation
> compared with the 4.0 alphas. Read the [migration and release notes](RELEASES.md)
> before updating dependencies or persisted data. Pin the exact alpha version;
> public APIs and data formats may change between alpha releases.
...
[Documentation index](README.md) · [API reference](https://docs.rs/physics_in_parallel/4.1.0-alpha)

## Installation

Use the exact alpha version:
...
PiP 4.1 alpha intentionally provides no compatibility aliases, migration wrappers,
...
Normal runs retain compiler auto-vectorization. Add `--no-autovectorize` only
for an explicit SIMD versus strictly scalar experiment. See
[the SIMD test guide](SIMD.md) for coverage and measurement details.
...
See [the correctness test guide](CORRECTNESS.md) for coverage and tolerances.
...
benchmark commands and hardware-validation limits; [advanced API guide](ADVANCED.md)
