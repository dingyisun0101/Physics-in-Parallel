...
### Batch 8 — large-data tuning

- Batch 7 pushed as `eff5fd5`. User explicitly prioritized large data over small-case optimization.
- Added a separate --large benchmark profile: one/ten-million-element arrays, 512×512 matrix multiplication, a 2048×2048 lattice, one million 3D particles, and 250,000-particle neighbor rebuild/query. Captured the validated pre-tuning baseline before changing library code.
- Fused allocating f32/f64 scale into one source-to-output pass using AVX2/AVX-512F or portable loops. Workers initialize disjoint spare-capacity chunks; vector length is exposed only after all chunks complete. Added scalar-edge, unaligned and tail checks for the new paths.
- Added SIMD Euler kernels over flat all-active particle blocks, preserving separate multiply/add and explicit old-velocity semantics. Mixed masks retain the row path. Added a large-state equation and cross-pool replay test.
- Large kinetic summaries now compute fixed 16,384-particle blocks in bounded worker groups and combine results in block order. Grouping and first-invalid-block diagnostics do not depend on worker count; documented the rounding difference versus a single sequential fold. Replaced Rayon minimum-length splitting after observing that it could underuse the allowed workers on non-power-of-two block counts.
- Replaced per-cell neighbor vectors with reusable sorted (cell, particle) records, compact ranges and hash lookup. Sorted cells preserve traversal order independently of the hash table. Fixed an internal helper's slice type during the refactor; all debug tests then passed. Added warmed allocation checks showing no rebuild allocations and only two dimension-sized query buffers.
- Initial four-worker large measurements: ten-million-value allocating scale approximately 58.6 -> 23.6 ms; 250,000-particle neighbor rebuild/query approximately 212 -> 104 ms. Euler remained memory-bandwidth limited on this AVX2 host, so no Euler speedup is claimed. Final timing and release verification follow the improved kinetic worker grouping.

- Final large run recorded in benches/results/2026-09-04-large-{before,after}.csv. Four-worker medians: allocating scale at 10,000,003 values 58.600 -> 21.630 ms; kinetic summary at one million 3D particles 7.798 -> 2.737 ms; neighbor rebuild/query at 250,000 particles 211.962 -> 106.347 ms. Explicit Euler 3.437 -> 2.526 ms after moving mask scans into worker chunks. BENCHMARKS.md also retains single-thread results and cautions against attributing unchanged-kernel timing variation to code changes.
- Final SIMD disassembly includes AVX2/AVX-512F Euler and source-to-output scale kernels. Tests exercise supported dispatch, IEEE edge classes, signed zero, unaligned starts, tails, large mixed-mask chunks, fixed-block observation replay and warmed neighbor storage reuse.
- All 67 Rust tests passed in debug/release during this batch, followed by focused reruns for final mask-group changes; strict Clippy, doctests, Python tests and warning-free rustdoc passed. Final review corrected overly broad span validation: ClampBox needs finite ordered endpoints but no representable subtraction, while periodic/reflection retain their required-span checks. Its constructor regression passed in debug/release before delivery.

- Final formatting, strict Clippy, warning-free rustdoc and whitespace checks passed. Committing and pushing the large-data tuning batch with raw before/after measurements and explicit hardware/numerical limits.
